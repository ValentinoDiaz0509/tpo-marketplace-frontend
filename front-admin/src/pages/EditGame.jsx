...
const EditGame = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/games/admin/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setTitle(data.title);
        setPrice(data.price);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        response = await fetch(`http://localhost:8080/games/admin/${id}/edit-with-image`, {
          method: "PUT",
          body: formData,
        });
      } else {
        // Si no cambia la imagen
        response = await fetch(`http://localhost:8080/games/admin/${id}`, {
          method: "PUT",
      ...
      <form onSubmit={handleSubmit}>
        ...
        {previewUrl ? (
          <div style={{ marginTop: "10px" }}>
            <img src={previewUrl} alt="Nueva vista previa" style={{ width: "100%", borderRadius: "10px" }} />
          </div>
        ) : existingImage ? (
          <div style={{ marginTop: "10px" }}>
            <img
