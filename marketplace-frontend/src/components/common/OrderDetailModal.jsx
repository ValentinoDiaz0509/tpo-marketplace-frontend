// src/components/OrderDetailModal.jsx (CÓDIGO MODIFICADO)
...
export default function OrderDetailModal({ order, isOpen, onClose }) {
  ...
  // Selector para obtener el juego actualmente cargado por fetchGameById (si es necesario)
  // Pero es más robusto manejar la promesa localmente para evitar conflictos con otros componentes.
  ...
  useEffect(() => {
    ...
    const fetchDetailsPromises = order.orderDetailResponses.map(
      async (detail) => {
        ...
        // Aquí estamos usando el thunk directamente para obtener el resultado de la promesa.
    ...
    Promise.all(fetchDetailsPromises)
      ...
      .catch((err) => {
        ...
        // Si hay error, mostrar solo los datos disponibles
        setDetailedItems(
