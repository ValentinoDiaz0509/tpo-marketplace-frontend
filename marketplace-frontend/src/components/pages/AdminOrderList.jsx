...
// NO IMPORTAMOS AdminLayout aquí, ya es el componente padre

const AdminOrderList = () => {
  const dispatch = useDispatch();
  // Extraemos el nuevo estado adminOrders
  const { adminOrders, loading, error } = useSelector((state) => state.orders); 

  useEffect(() => {
    // Si la lista está vacía, hacemos el fetch 
    if (adminOrders.length === 0) {
        dispatch(fetchAdminOrders());
    }
  }, [dispatch, adminOrders.length]);

  // Si loading o error, solo mostramos el contenido, sin envoltorio de layout
  if (loading) {
    return (
      <div className="p-6">
  ...
  return (
    // Ya no usamos <AdminLayout>, el contenido empieza aquí:
    <div className="p-6">
          ...
          <table className="min-w-full divide-y divide-gray-200 bg-white">
            <thead className="bg-[#32CD32] text-white">
              <tr>
                ...
                <th className="py-3 px-6 text-left text-sm font-semibold uppercase tracking-wider">Comprador</th>
                <th className="py-3 px-6 text-left text-sm font-semibold uppercase tracking-wider">Fecha</th>
                <th className="py-3 px-6 text-left text-sm font-semibold uppercase tracking-wider">Total</th>
                <th className="py-3 px-6 text-left text-sm font-semibold uppercase tracking-wider">Estado</th>
              </tr>
            </thead>
