...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("❌ Hubo un problema al procesar tu solicitud. Intenta nuevamente más tarde.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden font-display p-4 bg-background-dark">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center rounded-xl bg-surface-dark/80 p-8 backdrop-blur-md border border-border-color">
        ...
        <p className="text-text-secondary text-center pb-8">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <div className="w-full pt-4">
            <button
              type="submit"
              className="flex w-full items-center justify-center rounded-lg h-12 bg-primary text-background-dark font-bold"
              disabled={loading}
            >
