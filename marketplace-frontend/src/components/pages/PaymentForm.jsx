...
  }
  return (
    <div className="max-w-xl mx-auto p-6">
      ...
      <p className="text-sm text-gray-600 mb-4">Compra simulada para el juego ID: {gameId} — cantidad: {quantity}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input required value={cardNumber} onChange={e=>setCardNumber(e.target.value)} className="mt-1 block w-full border rounded p-2" placeholder="4111 1111 1111 1111" />
        </div>
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium">Expiración</label>
            <input required value={expiry} onChange={e=>setExpiry(e.target.value)} className="mt-1 block w-full border rounded p-2" placeholder="MM/AA" />
          </div>
          <div style={{width:120}}>
            <label className="block text-sm font-medium">CVV</label>
            <input required value={cvv} onChange={e=>setCvv(e.target.value)} className="mt-1 block w-full border rounded p-2" placeholder="123" />
          </div>
        </div>

        <div className="flex justify-between items-center mt-4">
          <button type="button" onClick={()=>navigate(-1)} className="px-4 py-2 border rounded">Cancelar</button>
          <button type="submit" disabled={loading} className="bg-green-600 text-white px-4 py-2 rounded">{loading ? 'Procesando...' : 'Pagar ahora'}</button>
        </div>
      </form>

      <p className="text-xs text-gray-500 mt-6">Nota: Este formulario es sólo una simulación — no se procesan pagos reales.</p>
    </div>
  )
}
